...
腳本會 build 所有 images，並：
- 將 image tar.gz 複製至 `~/release/images/`（供 portal Online 安裝下載）
- 同時產出 offline 交付包

產出：`~/release/neurosme-onprem-<版本>.tar.gz`

> **跳過 localauth build**：若 localauth 無更新，可加 `--skip-localauth` 複用上一版 image，節省 build 時間：
> ```bash
> bash ~/scripts/build-onprem.sh --skip-localauth
> ```
...
**換網路環境（搬遷展場、更換 IP / domain）：**
```bash
bash restart.sh --reconfigure
```
腳本會列出目前機器的 IP，重新詢問 domain 或 IP，更新設定後再重啟。
